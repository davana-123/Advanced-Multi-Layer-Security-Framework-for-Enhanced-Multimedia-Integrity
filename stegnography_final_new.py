...
def submit():
    ...
    lbl=tk.Label(win,text="Encryption Done",font=("elephant",10,"italic"),bg="#0e77bb",fg="black")
...
def decode_string():
    ...
    if str(video_file) == str(InputVideo):
        ...
        labelE=Label(win,text="Your Message is :  "+str(Msg),bg="#0e77bb",fg="black",font=("times",14,"bold"),height=3).place(x=1120,y=600)
        print('break {}'.format(secret))
        return "Your Message is :  "+str(Msg) , Msg
    else:
        print("File doesn't Matched") 
        return "Wrong Video Choosed " , "No Data"
...
def submit1():
    ...
    vid_name = f.read()
    f.close()
    location_new=labelot1.get()

    new_key = labelsk1.get()

    if old_key == new_key:
        ff,msg=decode_string()
        if msg == location_new:
            ff="Correct Location"
            Authenticated()
        else:
            if os.path.exists(vid_name):
                os.remove(vid_name)
                ff=f"File '{vid_name}' deleted successfully."
            else:
                ff=f"File '{vid_name}' not found."
    ...
    print(ff)
    labelR1=Label(win,text=ff,bg="#0e77bb",height=4,width=20,fg="black",font=("times",14,"bold"))
    labelR1.place(x=1000,y=450)
    labelR1=Label(win,text=msg,bg="#0e77bb",fg="black",font=("times",14,"bold"))
    labelR1.place(x=1000,y=550)
...
##    labelRF.configure(height=30)
    ...
    labels1=Label(win,text="Location",bg="#0e77bb",fg="black",font=("times",16,"bold"))
...
##    labelLF.configure(height=30)
    ...
    labels=Label(win,text="Location",bg="#0e77bb",fg="black",font=("times",16,"bold"))
